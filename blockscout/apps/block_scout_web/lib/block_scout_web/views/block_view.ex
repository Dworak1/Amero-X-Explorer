defmodule BlockScoutWeb.BlockView do
  ...
  def proposer_address(block_number) do
    validators =
      case Explorer.Repo.all(Explorer.Chain.Validator) do
        [] ->
          Application.get_env(:explorer, :polygon_edge_validators, [])

        list ->
          list
          |> Enum.sort_by(& &1.address_hash)
          |> Enum.map(&to_string(&1.address_hash))
      end

    if Enum.empty?(validators) do
      nil
    else
      # Determine proposer: (blockNumber % validatorCount)
      index = rem(block_number, length(validators))
      address_hash_string = Enum.at(validators, index)

      case Chain.string_to_address_hash(address_hash_string) do
        {:ok, hash} ->
          case Chain.find_or_insert_address_from_hash(hash) do
            {:ok, address} -> address
            _ -> %Explorer.Chain.Address{hash: hash, names: []}
          end
